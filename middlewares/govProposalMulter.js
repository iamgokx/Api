...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    if (file.fieldname === "media") {
      cb(null, path.join(__dirname, "../uploads/govProposalMedia"));
    } else if (file.fieldname === "documents") {
      cb(null, path.join(__dirname, "../uploads/govProposalFiles"));
    }
  },
...
const govProposalMulter = multer({
  storage,
  fileFilter,
  limits: { fileSize: 55 * 1024 * 1024 },
});

module.exports = { govProposalMulter };
